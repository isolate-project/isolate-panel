...
func NewDataRetentionService(db *gorm.DB, interval time.Duration) *DataRetentionService {
	if interval == 0 {
...
func (dr *DataRetentionService) cleanupOldData() {
	...
	// Raw stats: keep 7 days
	rawCutoff := now.AddDate(0, 0, -7)
	dr.db.Where("granularity = ? AND recorded_at < ?", "raw", rawCutoff).
		Delete(&models.TrafficStats{})

	// Hourly stats: keep 90 days
	hourlyCutoff := now.AddDate(0, 0, -90)
	dr.db.Where("granularity = ? AND recorded_at < ?", "hourly", hourlyCutoff).
		Delete(&models.TrafficStats{})

	// Daily stats: keep indefinitely (no cleanup)

	// Active connections: cleanup stale (older than 1 hour without activity)
	connCutoff := now.Add(-1 * time.Hour)
	dr.db.Where("last_activity < ?", connCutoff).
